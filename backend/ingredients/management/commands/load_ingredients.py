    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if Ingredient.objects.exists():
                self.stdout.write(
                    self.style.WARNING(
                        "Ингредиенты уже существуют в базе данных."
                        " Пропускаем импорт."
                    )
                )
                return
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    ...
                    f"{ingredient_count} ингредиентов"
                )
            )

        except json.JSONDecodeError:
            self.stdout.write(self.style.ERROR(
                f"Неверный формат JSON в {file_path}"))
